...
//echo $image['F_directory'];
    ...
    <div class="caption">
        ...
        <h4 style="font-size: 14px; min-height: 2.2em"><?php echo $row['TE_name']; ?>, <?php echo $row['B_name']; ?></h4>
        ...
        <div class="clearfix cart-button button-group">
            <div clas="row">
                ...
                if (count($row['OE_IMAGES']['OTHERS']) > 0) {
                    foreach ($row['OE_IMAGES']['OTHERS'] as $image) {
                        $l = '<a title="Брошюра" data-rel="shadowbox;player=iframe" class="btn btn-wishlist" href="http://new.komimport.ru/pdfjs/web/viewer.html?file=' . ('http://partist.ru/' . $image['F_directory'] . '/' . $image['F_file']) . '#locale=ru"> <i class="fa fa-file-pdf-o"></i></a>';
                        $b = true;
                ...
                if ($b) {
                    echo '<div style="padding-left: 0px;" class="col-sm-3">' . $l . '</div>';
                }
                ?>


                <div style="padding-left: 0px;padding-right: 0px;" class="col-sm-<?php echo $b == true ? 9 : 12; ?>">
                    ...
                    <button style="display: block; width: 100%" type="button" data-toggle="modal" data-target="#myModal_<?php echo $row['OE_id']; ?>" class="btn btn-cart">
                        Заказать
